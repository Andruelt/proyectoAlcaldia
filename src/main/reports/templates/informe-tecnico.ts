...
import type { ActividadRef, ReportContext, ReportField, ReportTemplate } from '../types';
import { InformeTecnicoDocx } from './informe-tecnico-docx';
import { InformeTecnicoPdf } from './informe-tecnico-pdf';
...
export const DEFAULT_COMPONENTES: ComponenteItem[] = [
...
export const COMPONENTE_ESTADO_OPTIONS = [
    { value: 'OPERATIVO', label: 'OPERATIVO' },
    { value: 'NO OPERATIVO', label: 'NO OPERATIVO' },
];

const INFORME_TECNICO_FIELDS: ReportField[] = [
    { key: 'fecha', label: 'Fecha', type: 'date', required: true, defaultValue: () => todayIso(), group: 'Encabezado' },
    { key: 'numeroInforme', label: 'N° de Informe', type: 'text', placeholder: '042', required: true, group: 'Encabezado' },
    { key: 'destinatarioNombre', label: 'Destinatario (Nombre)', type: 'text', placeholder: 'Abg. KARLA SOCORRO', required: true, group: 'Destinatario' },
    { key: 'destinatarioCargo', label: 'Cargo', type: 'text', placeholder: 'DIRECTORA DE RRHH', group: 'Destinatario' },
    { key: 'destinatarioDpto', label: 'Departamento / Alcaldía', type: 'text', placeholder: 'ALCALDIA DE INFANTE', group: 'Destinatario' },
    { key: 'codigoBien', label: 'Código del Bien', type: 'text', placeholder: 'REGU-0049', group: 'Equipo' },
    { key: 'descripcion', label: 'Descripción del equipo', type: 'longtext', rows: 3, placeholder: 'Regulador, Modelo Integra, Color Beige y Violeta', group: 'Equipo' },
    { key: 'requerimiento', label: 'Requerimiento', type: 'longtext', rows: 3, placeholder: 'Diagnóstico de equipos en condición: NO OPERATIVO', group: 'Equipo' },
    ...
    { key: 'firma1', label: 'Firma 1', type: 'text', placeholder: 'INFORMATICA Y SISTEMA', group: 'Firmas' },
    { key: 'firma2', label: 'Firma 2', type: 'text', placeholder: 'DIVISION DE REGISTRO Y CONTROL DE BIENES', group: 'Firmas' },
    { key: 'firma3', label: 'Firma 3', type: 'text', placeholder: 'RECIBIDO POR:', group: 'Firmas' },
];
...
const inferNumero = (actividad: ActividadRef | undefined): string => {
    if (!actividad?.id) return '';
    return actividad.id.replace(/-/g, '').substring(0, 6).toUpperCase();
};

function getComponentes(data: Record<string, unknown>): ComponenteItem[] {
    const raw = data.componentes;
    if (Array.isArray(raw) && raw.length > 0) return raw as ComponenteItem[];
    return [];
}

function shouldShowComponents(data: Record<string, unknown>): boolean {
    return getComponentes(data).length > 0;
}

const renderPreview = (data: Record<string, unknown>, ctx: ReportContext): string => {
...
<article class="informe-doc informe-doc--informe-tecnico">
    ...
    ${showComponents ? `
    <p class="it-greeting it-greeting--compact">${showComponents && !diag ? 'Componentes del equipo:' : ''}</p>
    ${componentRowsHtml}` : (diag ? `
    <table class="it-equipment">
        <tbody>
            <tr>
...
export const InformeTecnicoTemplate: ReportTemplate = {
    ...
    renderPreview,
    renderPdfHtml: (data, ctx) => InformeTecnicoPdf(data, ctx, renderPreview),
    buildDocx: (data, ctx) => InformeTecnicoDocx(data, ctx),
};
